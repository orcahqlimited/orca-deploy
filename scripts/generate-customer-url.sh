...
echo "    • Azure CLI 2.60+ — https://aka.ms/installazurecli"
echo "    • Logged in:        az login"
echo ""
echo "  Install & run (PowerShell):"
echo ""
echo "    New-Item -ItemType Directory -Force -Path \"\$HOME\\orca\" | Set-Location"
echo "    curl -o orca.tar.gz \"${SAS_URL}\""
echo "    tar xzf orca.tar.gz"
echo "    cd orca-deploy"
echo "    npm install --production"
echo "    node dist/index.js"
echo ""
echo "  Install & run (macOS / Linux):"
echo ""
echo "    mkdir -p ~/orca && cd ~/orca"
echo "    curl -o orca.tar.gz \"${SAS_URL}\""
